...
- Parameter store tracks amplitude, frequency, warp, ridge strength, X/Y/Z offsets, Y/Z per-axis scaling, wireframe, and autorotation so UI + scene stay in sync.
...
- Algorithm-specific sliders (Worley Shift / Blend, Curl Strength / Detail, Alligator Wave / Plateau) directly remap the sculpted surface so every noise mode has responsive controls.
...
- **Sliders:** Displacement (amplitude), Frequency, Roughness (octave falloff), Warp (domain warp), Ridge sharpening, Clamp Outside/Inside, Offset X/Y/Z, Scale Y/Z, and Seed all map directly to the modifier pipeline.
